...
class Controller:
    ...
    def fight(self, current_players: List[Player]) -> str:
        ...
        current_players[1].stamina = max(first_player_damage, 0)

        second_player_damage = current_players[1].stamina / 2
        current_players[0].stamina = max(second_player_damage, 0)
        winner = sorted(current_players, key=lambda w: -w.stamina)[0]
